...
Панель по умолчанию слушает `0.0.0.0:8787`, поэтому доступна и с самого
хоста, и с других устройств доверенной локальной сети. Автоматически открываемый
браузер использует безопасный локальный адрес `http://127.0.0.1:8787`.

Host-проверка по-прежнему включена: loopback, частные IPv4-адреса, IPv6 ULA и
link-local адреса принимаются автоматически только при wildcard bind. Для
локального DNS-имени вроде `jericho.lan` добавьте точное имя в
`server.allowed_hosts`. Публичные IP-адреса и посторонние имена без явного
...
Панель не имеет отдельной пользовательской аутентификации и управляет CLI с
полным доступом, поэтому публиковать её следует только в доверенную LAN или за
собственным reverse proxy с аутентификацией и TLS.

Уже существующий `nightshift.toml` считается явной настройкой и не
перезаписывается обновлением. Если он был создан старой версией, один раз
замените `host = "127.0.0.1"` на `host = "0.0.0.0"`.
