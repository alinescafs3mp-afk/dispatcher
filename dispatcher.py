...
class Dispatcher:
    ...
    async def _review(self, p: TaskPacket, worktree: Path, base: str, commits: list[str]) -> dict[str, Any]:
        prompt = f'''Act as accepting architect. Review the real repository in {worktree}. Task: {json.dumps(asdict(p), ensure_ascii=False)}. Base: {base}. Commits: {commits}. Read git diff {base}..HEAD and neighboring contracts. Do not edit. Return JSON only: {{"verdict":"ACCEPT","summary":"...","remaining_risks":[]}} or CHANGES_REQUESTED with required_changes, or BLOCKED with blocker.'''
